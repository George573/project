...
class Search:
    ...
    def extract_links(self, soup):
        ...
        for i in range(0, len(links) - 2): #last two links are accounts.google.com, support.google.com
                ...
                self.setings.file_handler.append_to_links_file((re.split(":(?=http)",links[i]["href"].replace("/url?q=","")))[0])
    ...
    def google_search(self, page_from = 1, page_to = 2, delay = 0.1):
        ...
        for page_number in range(self.setings.page_number, page_to):
            ...
            for position in range(self.setings.position, len(self.setings.clean_strings)):
                ...
                self.setings.file_handler.write_to_position_file(str(position) + " " + str(page_number))
                time.sleep(delay)
            self.position = 1
